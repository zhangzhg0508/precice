    ...
    int               rankReceiver)
{
  ...
  if (mesh.vertices().empty()) {
    PRECICE_DEBUG("Mesh to send is empty.");
    return;
  }
  PRECICE_DEBUG("Number of vertices to send: {}", numberOfVertices);

  // We have to send the vertexIDs later if there is connectivity information
  const bool hasConnectivity = !(mesh.edges().empty() && mesh.triangles().empty());

  std::vector<double> coords(static_cast<size_t>(numberOfVertices) * dim);
  std::vector<int>    globalIDs(numberOfVertices);
  std::vector<int>    vertexIDs(hasConnectivity ? numberOfVertices : 0);
  ...
  _communication->send(hasConnectivity, rankReceiver);
  if (!hasConnectivity) {
    PRECICE_DEBUG("Sending mesh has no connectivity information.");
    return;
  }

  // We need to send the vertexIDs first.
  ...
  // Send the triangles if we are in 3D
  if (dim == 3) {
    ...
    int         rankSender)
{
  ...
  if (numberOfVertices == 0) {
    PRECICE_DEBUG("Receiving Mesh is empty.");
    return;
  }
  ...
  if (!hasConnectivity) {
    PRECICE_DEBUG("Receiving mesh has no connectivity information.");
    return;
  }

  // We first receive the original vertexIDs and build the vertex map
  ...
  if (numberOfEdges > 0) {
    ...
    for (int i = 0; i < numberOfEdges; i++) {
      ...
      mesh::Vertex *v0 = vertexMap[edgeIDs[i * 2]];
      mesh::Vertex *v1 = vertexMap[edgeIDs[i * 2 + 1]];
      mesh.createEdge(*v0, *v1);
    }
  }

  // Receive the triangles if we are in 3D
  if (dim == 3) {
    ...
    if (numberOfTriangles > 0) {
      ...
      for (int i = 0; i < numberOfTriangles; i++) {
        ...
        mesh::Vertex *v0 = vertexMap[triangleIDs[i * 3]];
        mesh::Vertex *v1 = vertexMap[triangleIDs[i * 3 + 1]];
        mesh::Vertex *v2 = vertexMap[triangleIDs[i * 3 + 2]];
...
void CommunicateMesh::broadcastSendMesh(const mesh::Mesh &mesh)
{
  ...
  if (mesh.vertices().empty()) {
    PRECICE_DEBUG("Mesh to broadcast is empty.");
    return;
  }
  PRECICE_DEBUG("Number of vertices to broadcast: {}", numberOfVertices);

  // We have to broadcast the vertexIDs later if there is connectivity information
  const bool hasConnectivity = !(mesh.edges().empty() && mesh.triangles().empty());
  ...
  // Is there connectivity?
  _communication->broadcast(hasConnectivity);
  if (!hasConnectivity) {
    PRECICE_DEBUG("Broadcasting mesh has no connectivity information.");
    return;
  }

  // We need to broadcast the vertexIDs first.
  ...
  // broadcast the triangles if we are in 3D
  if (dim == 3) {
    int numberOfTriangles = mesh.triangles().size();
...
void CommunicateMesh::broadcastReceiveMesh(
    mesh::Mesh &mesh)
{
  constexpr Rank rankBroadcaster{0};
  PRECICE_TRACE(mesh.getName(), rankBroadcaster);
  int dim = mesh.getDimensions();
  ...
  // We first broadcast the original vertexIDs and build the vertex map
  std::vector<int> vertexIDs;
  ...
  if (dim == 3) {
    ...
    PRECICE_DEBUG("Number of Triangles to recevie: {}", numberOfTriangles);
